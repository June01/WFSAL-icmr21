...
tiou_thre_dict = {'THUMOS14': np.linspace(0.1, 0.9, 9),
                   'ActivityNet12': np.linspace(0.1, 0.95, 18)}
...
def eval_mAP(ground_truth_filename, prediction_filename, datasetname='THUMOS14',
         subset='test', tiou_thresholds=np.linspace(0.1, 0.9, 9),
         verbose=True, check_status=False, eval_class=[], eval_vid=[]):
    
    DATASET_NAME = datasetname
    tiou_thresholds = tiou_thre_dict[datasetname]
  
    anet_detection = ANETdetection(ground_truth_filename, prediction_filename,
                                   subset=subset, tiou_thresholds=tiou_thresholds,
                                   verbose=verbose, check_status=False, eval_class=eval_class, eval_vid=eval_vid)
                                   # verbose=verbose, check_status=(DATASET_NAME == 'ActivityNet12'))
