...
def eval_acc(ground_truth_filename, prediction_filename, datasetname='THUMOS14',
         subset='test', verbose=True, check_status=False, eval_class=[]):

    DATASET_NAME = datasetname

    anet_classification = ANETclassification(ground_truth_filename, prediction_filename,
                                   subset=subset, verbose=verbose, check_status=False, eval_class=eval_class)
    # verbose=verbose, check_status=(DATASET_NAME == 'ActivityNet12'))
    return anet_classification.evaluate()
